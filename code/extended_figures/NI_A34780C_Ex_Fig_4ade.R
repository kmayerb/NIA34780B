# NI_A34780C_Ex_Fig_4ade.R

# NI_A34780C Now used for Ext Fig 4a, d, e
### R code for Extended Figure 5a,de
## clear env
rm(list=ls())
...
# Figure Data
ef5a_fdata = file.path(repo_loc, repo, 'figure_data_files','NI_A34780C_ex_fig_4a.data.csv')
ef5d_fdata = file.path(repo_loc, repo, 'figure_data_files','NI_A34780C_ex_fig_4d.data.csv')
ef5e_fdata = file.path(repo_loc, repo, 'figure_data_files','NI_A34780C_ex_fig_4e.data.csv')
...
a$data %>% 
  dplyr::select(pubid = ptid,
              ...
              enrich_type,
              cell_type, 
              `10x_pfreq`= X10x_pfreq,
              productive_frequency) %>% 
  write.csv(ef5a_fdata, row.names= FALSE)
...
d$data %>% 
       select(pubid = ptid, 
              ...
              detected, 
              e03_pfreq=E03_pfreq) %>% 
  write.csv(ef5d_fdata, row.names = F)
  
e$data %>% select(pubid = ptid, 
                  key = key.x, 
                  predetected, 
                  detected,
                  e01_to_e03_fold_change = E03_vac_fc) %>%
  write.csv(ef5e_fdata, row.names = F)
